...
export default function App() {
  ...
  const transitionBoxToReveal = (slotIndex: number, result: any) => {
    setMultiGame(prev => {
      ...
      const isWin = result.type === 'jackpot' || result.type === 'item' || result.type === 'consolation' || result.type === 'rescue';
      if (isWin) {
        playSound(SOUND_SUCCESS);
        if (result.type === 'jackpot' || result.type === 'item') {
            confetti({ particleCount: 100, spread: 60, origin: { x: (slotIndex % 3) * 0.33 + 0.16, y: Math.floor(slotIndex / 3) * 0.33 + 0.2 } });
        }
      } else {
        playSound(SOUND_SHATTER);
      }
    ...
    <div className={cn("min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#131b2f] via-[#0a0f1d] to-[#04060b] pb-24 font-sans select-none text-slate-200 transition-all duration-300 relative overflow-x-hidden", screenShake && "animate-shake-screen")}>
      ...
      <main className="p-4 max-w-md mx-auto">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {view === 'cart' && <CartView items={cart} onUpdateCount={(boxId, delta) => setCart(prev => prev.map(i => i.boxId === boxId ? {...i, count: Math.max(1, i.count + delta)} : i))} onRemove={(boxId) => setCart(prev => prev.filter(i => i.boxId !== boxId))} onCheckout={() => toast('Đang xử lý thanh toán...', 'info')} onBack={() => setView('home')} />}
            ...
            <div className="relative flex items-center justify-center mt-[-10%]">
               ...
               {interactiveState.phase === 'grand-reveal' && (
                 <motion.div
                    ...
                    className="w-64 h-80 bg-gradient-to-b from-yellow-100 to-yellow-500 rounded-3xl p-4 flex flex-col items-center justify-center shadow-[0_0_150px_rgba(250,204,21,1)] border-4 border-white relative z-50 inner-glow"
                 >
                    <div className="absolute inset-0 overflow-hidden rounded-2xl pointer-events-none">
                       <div className="w-[300%] h-[300%] absolute -top-[100%] -left-[100%] bg-[conic-gradient(from_0deg,transparent_0_340deg,rgba(255,255,255,0.9)_360deg)] animate-[spin_2s_linear_infinite]" />
                    </div>
                    <Smartphone className="w-24 h-24 text-red-700 drop-shadow-[0_10px_20px_rgba(0,0,0,0.4)] relative z-10 animate-bounce-subtle" />
                    <h3 className="mt-8 text-2xl font-black text-red-800 uppercase italic tracking-tighter relative z-10 drop-shadow-md text-center leading-tight">
                        {interactiveState.apiResult?.results?.[0]?.result?.split('+')[0] || 'SIÊU PHẨM'} 
                        <br/><span className="text-xl px-2 bg-red-800 text-yellow-300 tracking-widest rounded mt-1 inline-block">ĐẠN CHƯA BAY</span>
                    </h3>
                 </motion.div>
               )}
                      ...
                      {box.mode === 'win-reveal' && (
                        <motion.div
                           ...
                           className={cn(
                             ...
                             box.result?.type === 'jackpot' ? "bg-gradient-to-b from-yellow-200 to-yellow-500 border-yellow-300" :
      ...
      <AnimatePresence>
        {result && (
          ...
          >
            {(result.toLowerCase().includes('iphone') || result.toLowerCase().includes('ipad')) ? (
              // SUCCESS - VÀNG KIM
              <motion.div 
                initial={{ scale: 0.5, y: 100 }}
                animate={{ scale: 1, y: 0 }}
                className="w-full max-w-sm bg-gradient-to-b from-yellow-300 to-yellow-600 p-8 rounded-[4rem] border-4 border-white shadow-[0_0_150px_rgba(234,179,8,0.8)] text-center relative"
              >
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-white p-4 rounded-3xl shadow-2xl border-4 border-yellow-500">
                  <Smartphone className="w-12 h-12 text-yellow-600" />
                </div>
                
                <h2 className="text-4xl font-black text-red-700 uppercase italic tracking-tighter mt-4 leading-none">BẠN ĐÃ TRÚNG <br/> SIÊU PHẨM!</h2>
                <div className="my-8 bg-black/10 backdrop-blur-sm p-6 rounded-3xl border-2 border-white/20">
                  <p className="text-[10px] text-red-900 font-black uppercase tracking-widest mb-1 opacity-60">Phần thưởng:</p>
                  <p className="text-xl font-black text-white uppercase tracking-tighter drop-shadow-md leading-tight">{result}</p>
                </div>
               ...
               <motion.button 
                  ...
                  onClick={async () => {
                      ...
                      try {
                         ...
                         const d = await res.json();
                         if(d.success) {
                            toast(`Lắc quà thành công! +${d.reward.toLocaleString()}đ`);
                            playSound(SOUND_WIN);
                ...
                <>
                  ...
                  <div className="w-full space-y-3 relative z-10 px-2 lg:px-4 pb-4">
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      <button 
                        onClick={() => onQuickOpen(box.id, 5)}
                        className="w-full bg-red-500/20 text-red-100 font-black py-3 rounded-2xl shadow-xl hover:bg-red-500/30 uppercase text-xs sm:text-sm tracking-widest active:scale-[0.98] transition-all"
                      >
                        KHUI NHANH
                      </button>
                    </div>
